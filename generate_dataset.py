...
def sysrand_code(p):
    ...
    return f"""import std/sysrand

proc generateSecureToken*(): seq[byte] =
  result = urandom({s})

when isMainModule:
  let token = generateSecureToken()
  echo "Generated ", token.len, " secure random bytes"
"""

register_template("stdlib_sysrand", ["stdlib", "sysrand", "security"],
    [
        "Write a Nim program using std/sysrand to generate {size} secure random bytes.",
        "Create a script generating a secure token of {size} bytes using urandom.",
        "Implement a cryptographically secure random generator for {size} bytes.",
        "Build a Nim module that uses std/sysrand to get {size} random bytes."
    ],
    "This example uses the `sysrand` standard library module which provides cryptographically secure pseudo-random number generation. We call `urandom({size})` to obtain a sequence of securely generated bytes, suitable for tokens, keys, or nonces.",
    sysrand_code, sysrand_gen)
...
def generate_examples(target_total=3000):
    ...
    for t in TEMPLATES:
        ...
        secrets.SystemRandom().shuffle(perms)
        ...
        for p in perms[:target_per_t]:
            prompt_fmt = secrets.SystemRandom().choice(t.prompts)
